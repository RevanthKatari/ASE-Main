...
def seed():
    ...
    existing_helper = User.query.filter_by(email="helper@unilife.ca").first()
    if existing_helper:
        print("ℹ️  Helper account already exists. Skipping seed.")
        return

    helper = User(full_name="Community Helper", email="helper@unilife.ca", role="helper")
    helper.set_password("Password123!")

    student = User(full_name="International Student", email="student@unilife.ca", role="student")
    ...
    print("   Helper account: helper@unilife.ca / Password123!")
    print("   Student account: student@unilife.ca / Password123!")


if __name__ == "__main__":
    # Only create app when running as script
